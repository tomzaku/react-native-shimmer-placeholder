...
const getOutputRange = (width, isReversed) => {
    const [start, end] = typeof width === 'string' ? ['-100%', '100%'] : [-width, width]

    return isReversed ? [end, start] : [start, end]
}

const getShimmerWidth = (width, shimmerWidthPercent) => {
    return typeof width === 'string'
        ? `${100 * shimmerWidthPercent}%`
        ...
        >
          ...
          >
            <LinearGradient
              ...
              style={{ flex: 1, width: getShimmerWidth(width, shimmerWidthPercent) }}
              start={{
                x: -1,
                y: 0.5,
              }}
